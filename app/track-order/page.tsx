...
function TrackOrderContent() {
  ...
  const generateTimeline = (status: string, date: string) => {
    ...
    return [
      ...
      {
        label: "Processing",
        date: currentLevel >= 1 ? "Done" : "Pending",
        done: currentLevel >= 1,
        icon: <Clock className="w-4 h-4" />,
      },
      {
        label: "Shipped",
        date: currentLevel >= 2 ? "Done" : "Pending",
        done: currentLevel >= 2,
        icon: <Truck className="w-4 h-4" />,
      },
      {
        label: "In Transit",
        date: currentLevel >= 3 ? "Done" : "Pending",
        done: currentLevel >= 3,
        icon: <MapPin className="w-4 h-4" />,
      },
      {
        label: "Delivered",
        date: currentLevel >= 4 ? "Delivered" : "Est. Delivery",
        done: currentLevel >= 4,
  ...
  const fetchOrder = async (searchQuery: string) => {
    ...
    try {
      const apiUrl = `https://my-shop-t2x7.vercel.app/api/orders/track/${trimmed}`;
      ...
      if (data.success && data.data) {
        ...
        const dateStr =
          orderData.createdAt || orderData.date || new Date().toISOString();
  ...
  const handleTrack = () => fetchOrder(query);
            ...
            <Card className="border-destructive/30 bg-destructive/5 animate-fade-in">
              <CardContent className="p-8 text-center">
                ...
                <p className="text-muted-foreground text-sm">
                  We couldn't find an order with that order number. Please double-check and try again.
                </p>
              </CardContent>
            </Card>
          )}
          ...
          {result && (
            <div className="space-y-6 animate-fade-in">
              ...
              <Card className="overflow-hidden border-border/60 shadow-md">
                <CardHeader className="bg-muted/50 border-b pb-4">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    ...
                    <span
                      className={`text-sm font-bold px-4 py-1.5 rounded-full border capitalize shrink-0 ${statusColor(result.status)}`}
                    >
                      {result.status}
                    </span>
                  </div>
                ...
                <CardContent className="p-0">
                  <div className="divide-y divide-border">
                    {result.items?.map((item: any, idx: number) => (
                      <div
                        key={idx}
                        className="flex flex-col sm:flex-row p-5 sm:p-6 gap-5"
                      >
              ...
              <Card className="border-border/60 shadow-md">
                <CardContent className="p-6 sm:p-8">
                  <h3 className="font-heading font-bold text-lg text-foreground mb-8 border-b pb-4">
                    Shipment Progress
                  </h3>
                    ...
                    {result.timeline.map((step, i, arr) => (
                      <div key={i} className="flex gap-5">
                        ...
                        <div className="flex flex-col items-center">
                          <div
                            className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2 transition-colors shadow-sm ${
                              step.done
                          ...
                          {i < arr.length - 1 && (
                            <div
                              className={`w-0.5 h-12 my-1 ${step.done ? "bg-primary/50" : "bg-border"}`}
                            />
                          )}
                        </div>
                        {/* Info */}
                        <div className="pt-2 pb-6">
                          <p
                            className={`text-base font-bold ${step.done ? "text-foreground" : "text-muted-foreground"}`}
                          >
                            {step.label}
                          </p>
