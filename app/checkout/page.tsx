...
export default function CheckoutPage() {
  ...
  if (isLoading) {
    return (
      <div className="container mx-auto px-4 py-12 min-h-screen max-w-6xl">
         <div className="h-8 w-48 animate-pulse rounded mb-10"></div>
  ...
  // Prevent rendering if cart is empty and modal is not showing
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-10 max-w-6xl">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
            ...
            <div>
              ...
              <form id="checkout-form" onSubmit={handlePlaceOrder} className="space-y-5">
                <div className="space-y-2">
                  ...
                  <Input id="fullName" name="fullName" placeholder="Enter your full name" className="h-12" onChange={handleInputChange} required />
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="phone">Phone Number</Label>
                  <Input id="phone" name="phone" placeholder="e.g. +880 1712-345678" className="h-12" onChange={handleInputChange} required />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="address">Full Address</Label>
                  <Textarea id="address" name="address" placeholder="House number, street name, area" className="min-h-[100px] resize-none" onChange={handleInputChange} required />
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div className="space-y-2">
                    <Label htmlFor="city">City / District</Label>
                    <Input id="city" name="city" placeholder="e.g. Dhaka" className="h-12" onChange={handleInputChange} required />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="country">Country</Label>
                    <Input id="country" name="country" value="Bangladesh" disabled className="h-12 bg-muted/50 text-muted-foreground" />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="border-2 border-[#16a34a] rounded-xl p-5 flex items-center justify-between bg-[#16a34a]/5 transition-all">
                    <div className="flex items-center gap-4">
                      <div className="h-6 w-6 rounded-full border-2 border-[#16a34a] flex items-center justify-center">
                        <div className="h-3 w-3 rounded-full bg-[#16a34a]" />
                      </div>
                      <div>
                        <p className="font-bold text-gray-900">Cash on Delivery</p>
                        <p className="text-sm text-muted-foreground mt-0.5">Pay with cash upon delivery.</p>
                      </div>
                    </div>
                    <Truck className="h-8 w-8 text-[#16a34a] opacity-80" />
                  </div>
                </div>
              </form>
            </div>
          ...
          <div className="order-1 lg:order-2 lg:col-span-5 lg:sticky lg:top-24">
            <div className="rounded-2xl border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
              ...
              <div className="p-6">
                <div className="space-y-5 max-h-[320px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-200 mb-6">
                  {cart.map((item, idx) => (
                    <div key={idx} className="flex gap-4 items-center group">
                      <div className="relative h-16 w-16 rounded-xl overflow-hidden border border-gray-100 bg-white shrink-0">
                        <Image src={item.image} alt={item.name} fill className="object-cover" />
                        <div className="absolute -top-2 -right-2 h-5 w-5 bg-gray-900 text-white rounded-full flex items-center justify-center text-[10px] font-bold z-10 border-2 border-white">
                          {item.quantity}
                        </div>
                      </div>
                ...
                <div className="flex justify-between items-center mb-8">
                  ...
                  <span className="text-2xl font-black text-[#16a34a]">৳{cartTotal.toLocaleString()}</span>
                </div>
      ...
      <Dialog open={showSuccessModal}>
        <DialogContent className="sm:max-w-md text-center p-8 border-none shadow-2xl rounded-2xl" hideCloseButton>
          <DialogHeader className="flex flex-col items-center gap-3">
            ...
            <DialogTitle className="text-2xl font-extrabold text-gray-900">Order Confirmed!</DialogTitle>
          ...
          <div className="bg-gray-50 p-5 rounded-xl border border-gray-100 my-4">
            <p className="text-sm text-gray-500 font-medium mb-1">Your Order ID</p>
            <p className="text-xl font-mono font-black text-[#16a34a] tracking-wider">{generatedOrderId}</p>
          </div>
